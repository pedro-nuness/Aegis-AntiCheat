...
File::File( std::string _filename )
{
	this->FilePath = "";
	this->FileName = _filename;
	this->FullPath = this->FilePath + _filename;
}
...
int File::GetNumLines() {
    // Se as linhas precisarem ser atualizadas
    if (_NeedLinesUpdate) {
        int CountLines = 0;
        ...
        if (_file.is_open()) {
            ...
            _file.close(); // Garante que o arquivo foi fechado após leitura
            _Lines = CountLines; // Atualiza o número de linhas
            _NeedLinesUpdate = false; // Desmarca a necessidade de atualizar
        } else {
            // Se o arquivo não pôde ser aberto, retorna 0 ou trata o erro conforme necessário
            std::cerr << "Erro ao abrir o arquivo para contagem de linhas." << std::endl;
            return 0;
        }
    }

    // Se não precisar atualizar, retorna a quantidade de linhas já armazenada
    return _Lines;
}
