...
)
{
    ...
    if (!pNtHeaders)
    {
        printf( xorstr_( "RtlImageNtHeader failed. (BaseAddress = 0x%IX)\n" ) ,
            ImageBase);
        status = FALSE;
        goto exit;
    }
    ...
    if (!pRemapRegion)
    {
        printf( xorstr_( "VirtualAlloc failed: %u\n" ) , GetLastError());
        status = FALSE;
        goto exit;
    }
    ...
    if (!status)
    {
        printf( xorstr_("RmppRemapImageRoutine failed.\n" ) );
        goto exit;
    }
    ...
    if (!status)
    {
        printf( xorstr_( "RmppValidateRemappedImageProtection failed.\n" ) );
        goto exit;
    }
...
exit:
    if (pRemapRegion)
    {
        ...
        {
            printf( xorstr_( "VirtualFree failed: %u\n" ) , GetLastError());
        }
    }
...
)
{
    ...
    for ( WORD i = 0; i < EXPECTED_SECTIONS; ++i )
    {
     ...
     /*   printf( xorstr_( "Checking section %d: SectionBase = 0x%IX, VirtualAddress = 0x%X\n" ) ,
            i , SectionBase , pSectionHeader[ i ].VirtualAddress );*/
        ...
        if ( !status )
        {
            printf( xorstr_( "Unexpected section alignment. (SectionBase = 0x%IX)\n") , SectionBase );
            goto exit;
        }
    }
...
)
{
    ...
    {
        printf( xorstr_( "NtCreateSection failed: 0x%X\n" ) , ntstatus);
        status = FALSE;
        goto exit;
    }
    ...
    if (!NT_SUCCESS(ntstatus))
    {
        printf( xorstr_( "NtMapViewOfSection failed: 0x%X\n" ) , ntstatus);
        status = FALSE;
        goto exit;
    }
    ...
    if (!NT_SUCCESS(ntstatus))
    {
        printf( xorstr_( "NtUnmapViewOfSection failed: 0x%X\n" ) , ntstatus);
        status = FALSE;
        goto exit;
    }
    ...
    for (WORD i = 0; i < EXPECTED_SECTIONS; ++i)
    {
        ...
        if (!status)
        {
            printf( xorstr_( "RmppMapProtectedView failed.\n" ) );
            goto exit;
        }
    }
...
exit:
    if (hSection)
    {
        ...
        if (!NT_SUCCESS(ntstatus))
        {
            printf( xorstr_( "NtClose failed: 0x%X\n" ) , ntstatus);
        }
    }
...
)
{
    ...
    for (WORD i = 0; i < EXPECTED_SECTIONS; ++i)
    {
        ...
        if (!status)
        {
            printf( xorstr_( "RmppValidateRemappedPeSectionProtection failed.\n") );
            goto exit;
        }
    }
...
)
{
    ...
    {
        printf( xorstr_( "VirtualQuery failed: %u (BaseAddress = 0x%IX)\n" ) ,
            GetLastError(),
            pSectionBase);
        status = FALSE;
        goto exit;
