...
/// process 64 bytes
void CUSTOM_SHA::processBlock(const void* data)
{
    // get last hash
    uint32_t a = m_hash[0];
    uint32_t b = m_hash[1];
...
void CUSTOM_SHA::add(const void* data, size_t numBytes)
{
...
void CUSTOM_SHA::getHash(unsigned char buffer[CUSTOM_SHA::HashBytes])
{
...
/// compute CUSTOM_SHA of a memory block
std::string CUSTOM_SHA::operator()(const void* data, size_t numBytes)
{
    reset();
    add(data, numBytes);
    return getHash();
...
/// compute CUSTOM_SHA of a string, excluding final zero
std::string CUSTOM_SHA::operator()(const std::string& text)
{
    reset();
